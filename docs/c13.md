...
begin{enumerate}
	...
	item 减少输入的字符，因为用“}”关闭作用域比创建缓冲区的host_accessor更简洁(更容易出错)。
end{enumerate}
...
begin{itemize}
	item SYCL_EXTERNAL只能用于函数。
	item SYCL_EXTERNAL函数不能使用原始指针作为形参或返回类型。必须使用显式指针类。
	item SYCL_EXTERNAL函数不能调用parallel_for_work_item。
	item SYCL_EXTERNAL函数不能在parallel_for_work_group范围内调用。
end{itemize}
